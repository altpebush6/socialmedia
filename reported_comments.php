...
$posts = $db->getDatas("SELECT * FROM posts WHERE PostActive = ? AND CommentReport > 0 ORDER BY PostAddTime DESC", array(1));
...
?>
    <div class="container my-5 px-0 px-md-4" id="reppostid_<?= $post_ID ?>">
        <div class="border rounded-1 p-3 col-md-10 offset-md-1 py-4 post">
            <div class="row mb-3">
                ...
                <div class="col-3 text-end"> <a href="javascript:void(0)" class="fs-4 text-danger" title="Remove Comment's Report" onClick="RemoveCommentReport('removeCommentRep','<?= $post_ID ?>')"><i class="fas fa-eraser"></i></a></div>
            </div>
            ...
            <div class="row py-2 mt-2" id="likecomment_<?= $post_ID ?>">
                ...
                <div class="col-6 text-center text-dark" id="comment_<?= $post_ID ?>">
                    <a onClick="repComopenComments(<?= $post_ID ?>)">
            ...
            <!-- Gönderi Yorumları -->
            <div class="row p-2 justify-content-center" style="display:none;user-select:text" id="Commentrepcomments_<?= $post_ID ?>">
                <?php
                ...
                ?>
                    <div class="row text-dark px-3 py-1 mt-md-2 each_comment_<?= $comment_ID ?>">
                        ...
                        <div class="col-2 py-1 text-end d-flex align-item-center justify-content-end">
                            <?php
                            $reportcount = $postinfo->CommentReport;
                            if ($reportcount > 5) {
                                $reportclass = 'text-danger';
                            } else {
                                $reportclass = 'text-warning';
                            }
                            if ($reportcount > 0) { ?>
                                <span class="<?= $reportclass ?> fs-5 me-1"><?= $reportcount ?></span> <i class="fas fa-flag <?= $reportclass ?> pt-2" style="font-size:15px;"></i>
                            <?php }
                            ?>

                        </div>
                        <div class="col-1 pt-1 px-0 d-flex align-item-center">
                            <a href="javascript:void(0)" onClick="DeleteComment('deleteComment','<?= $comment_ID ?>','<?= $post_ID ?>')" class="fs-5 text-danger"><i class="far fa-trash-alt"></i></a>
                        </div>
                    </div>
                <?php } ?>
            </div>
