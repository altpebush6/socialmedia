<div class="container py-4 px-0 px-md-4">
  <div class="create-post shadow border col-md-10 offset-md-1 py-4 bg-light" style="border-radius: 15px;">
    <form id="form_posting" method="post" enctype="multipart/form-data" class="px-3">
      ...
      <div class="row mt-3">
        <div class="col-10 mx-auto">
          <button type="submit" class="btn w-100 shadow rounded-3 border fs-5 btn-post" name="submitpost" id="submitpost"><?= $translates["shareit"] ?> <span class="spinner" id="spinnershare"></span></button>
        </div>
      </div>
    </form>
  </div>
...
<?php
$posts = $db->getDatas("SELECT * FROM posts WHERE PostActive = ? AND PostClub = ?  ORDER BY PostAddTime DESC LIMIT 3", array(1, $part));

if (!$posts) { ?>
  <div class="container">
    <div class="row">
      <div class="col-10 mx-auto my-3 p-4 text-dark fs-5 text-center rounded-3 border"><?= $translates["nopostsofclub"] ?></div>
    </div>
  </div>
  <?php }
  ...
  ?>
    <div class="container my-5 px-0 px-md-4" id="<?= $post_ID ?>">
      <div class="border shadow bg-light p-3 col-md-10 offset-md-1 py-4 post" style="border-radius: 15px;">
          ...
          <div class="col-2">
            <div class="dropdown-post">
              ...
              <div class="dropdown-content" style="width:220px;">
                ...
                <?php
                } elseif ($club->ClubPresidentID == $memberid) { ?>
                  <a href="javascript:void(0)" onClick="DeletePost('deletepost','<?= $memberid ?>','<?= $post_ID ?>')"><i class="far fa-trash-alt"> <?= $translates["deletepost"] ?></i></a>
                  <?php } else {
                  $diduRep = $db->getData("SELECT * FROM reports_posts WHERE ReporterID = ? AND ReportedID = ?", array($memberid, $post_ID));
                  if ($diduRep) {
                  ?>
                    <a href="javascript:void(0)" class="text-success unreportPost" id="Report_Post_<?= $post_ID ?>" postid="<?= $post_ID ?>"><i class="fas fa-headset"></i> <?= $translates["reported"] ?></a>
                  <?php } else { ?>
                    <a href="javascript:void(0)" class="text-danger reportPost" id="Report_Post_<?= $post_ID ?>" postid="<?= $post_ID ?>"><i class="fas fa-bug"></i> <?= $translates["reportpost"] ?></a>
                <?php }
                } ?>
              </div>
            </div>
        ...
        <div class="text-dark text-break fs-6 px-5 my-4 postmiddle_<?= $post_ID ?>" style="user-select:text" id="postmiddle_<?= $post_ID ?>">
          ...
          <div class="row d-flex flex-column ps-3" id="post_files_<?= $post_ID ?>">
            ...
            for ($i = 1; $i < 5; $i++) {
              if ($i > 1) {
                $postfile = "PostFile" . $i;
                if ($item->$postfile) {
                  echo '<div class="col-12 my-2 ps-4 fs-6"><a class="text-dark" href="http://localhost/aybu/socialmedia/' . $translates["clubs"] . '?download=' . $item->$postfile . '"><i class="fas fa-file-alt fa-2x"></i> ' . $item->$postfile . '</a></div>';
                }
              } else {
                if ($item->PostFile) {
                  echo '<div class="col-12 my-2 ps-4 fs-6"><a class="text-dark" href="http://localhost/aybu/socialmedia/' . $translates["clubs"] . '?download=' . $item->PostFile . '"><i class="fas fa-file-alt fa-2x"></i> ' . $item->PostFile . '</a></div>';
            ...
            <div class="row w-100 ps-4" id="post_images_<?= $post_ID ?>">
              <?php if (!is_null($post_img)) {
                switch ($img_counter) {
                  ...
                  <?php break;
                  case 2: ?>
                    <a href="post_images/<?= $post_img ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border-end">
                      <img src="post_images/<?= $post_img ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img2 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center">
                      <img src="post_images/<?= $post_img2 ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                  <?php break;
                  case 3: ?>
                    <a href="post_images/<?= $post_img ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border">
                      <img src="post_images/<?= $post_img ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img2 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border">
                      <img src="post_images/<?= $post_img2 ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img3 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border">
                      <img src="post_images/<?= $post_img3 ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                  <?php break;
                  case 4: ?>
                    <a href="post_images/<?= $post_img ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border p-0 ">
                      <img src="post_images/<?= $post_img ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img2 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border p-0 ">
                      <img src="post_images/<?= $post_img2 ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img3 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border p-0 ">
                      <img src="post_images/<?= $post_img3 ?>" style="max-width:100%;min-width:100px;max-height:64vh;border-radius:5px;">
                    </a>
                    <a href="post_images/<?= $post_img4 ?>" class="col-6 pe-1 d-flex justify-content-center align-items-center border p-0 ">
                  ...
                  switch ($img_counter) {
                    case 1: ?>
                      <a href="post_images/<?= $post_img ?>" class="col-12" class="editting_img_<?= $post_ID ?>">
                        <img src="post_images/<?= $post_img ?>" style="width:100%;max-height:64vh;border-radius:5px;">
                      </a>
                    <?php break;
                    case 2: ?>
                      ...
                      <a href="post_images/<?= $post_img2 ?>" class="col-6 pe-1" class="editting_img_<?= $post_ID ?>">
                        <img src="post_images/<?= $post_img2 ?>" style="width:100%;max-height:64vh;border-radius:5px;">
                      </a>
                    <?php break;
                    case 3: ?>
                      <a href="post_images/<?= $post_img ?>" class="col-6 pe-1" class="editting_img_<?= $post_ID ?>">
                      ...
                      <a href="post_images/<?= $post_img3 ?>" class="col-6 pe-1" class="editting_img_<?= $post_ID ?>">
                        <img src="post_images/<?= $post_img3 ?>" style="width:100%;max-height:64vh;border-radius:5px;">
                      </a>
                    <?php break;
                    case 4: ?>
                      <a href="post_images/<?= $post_img ?>" class="col-6 pe-1" class="editting_img_<?= $post_ID ?>">
                      ...
                      <a href="post_images/<?= $post_img4 ?>" class="col-6 pe-1" class="editting_img_<?= $post_ID ?>">
                        <img src="post_images/<?= $post_img4 ?>" style="width:100%;max-height:64vh;border-radius:5px;">
                      </a>
                <?php break;
                  }
                } ?>
