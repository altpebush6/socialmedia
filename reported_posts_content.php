...
$posts = $db->getDatas("SELECT * FROM posts WHERE PostActive = ? AND PostReport > 0 ORDER BY PostAddTime DESC", array(1));
    ...
    <div class="border rounded-1 p-3 col-md-10 offset-md-1 py-4 post">
      <div class="row mb-3">
        ...
        <div class="col-2 py-1 text-end">
          ...
          ?>
          <span class="<?= $reportclass ?> fs-5"><?= $reportcount ?></span> <i class="fas fa-flag <?= $reportclass ?>" style="font-size: 21px"></i>
        </div>
        <div class="col-1 px-0 d-flex flex-column justify-content-center">
          <a href="javascript:void(0)" class="fs-4 text-danger" title="Remove Report" onClick="RemovePostReport('removePostRep','<?= $post_ID ?>')"><i class="fas fa-eraser"></i></a>
          <a href="javascript:void(0)" title="Remove Post" onClick="DeletePost('deletepost','<?= $post_ID ?>')" class="fs-4 text-danger"><i class="far fa-trash-alt"></i></a>
        </div>
      </div>
      ...
      <div class="row py-2 mt-2" id="likecomment_<?= $post_ID ?>">
        ...
        <div class="col-6 text-center text-dark" id="comment_<?= $post_ID ?>">
          <a onClick="repopenComments(<?= $post_ID ?>)">
      ...
      <!-- Gönderi Yorumları -->
      <div class="row p-2 justify-content-center" style="display:none;user-select:text" id="repcomments_<?= $post_ID ?>">
        <?php
