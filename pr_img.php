...
if(isset($_POST['deleteimg1'])){      // Eğer kapak fotoğrafını sil'i seçerse
    ...
    $cover_img_name = $db->getColumnData("SELECT Member_Coverimg FROM images WHERE MemberID = ?",array($memberid));
    ...
    $profile_img_name = $db->getColumnData("SELECT Member_Profileimg FROM images WHERE MemberID = ?",array($memberid));
